...
- Per-theme mobilePos (медведь center top, орёл/жук center center)
...
### Round 3 — мобильная вёрстка + SSG + RSS (2026-04-01)

**Медведь mobile — 3 разных кропа из Figma:**
- base: `bear-mobile.jpg` (960×2040, высокий портрет)
- xs: `bear-mobile-xs.jpg` (720×1093, средний портрет)
- sm: `bear-mobile-sm.jpg` (960×1089, почти квадрат)
- В HeroSection: `<picture>` с `<source media>` для переключения по брейкпоинтам
- Старый scaleY(1.21) translateY(-9%) УБРАН — теперь object-cover + center top

**Структура 1st screen (как в Figma):**
- Timer и Cards ВЫНЕСЕНЫ из content-wrapper в отдельные секции ниже
- Content-wrapper: min-h по Figma (base 484, xs 604, sm 660), НЕ flex-1 на mobile
- base/xs: justify-end, pb-20 — контент прижат к низу, медведь виден сверху
- sm: justify-start, pt-120, pb-60, items-center — контент позиционирован padding'ом
- md+: flex-1, pt/pb из десктопных Figma-значений

**RSS:**
- Убран allorigins.win прокси (нестабильный, 500/520)
- Свой `/api/rss.ts` — Vercel serverless, фетчит напрямую с vedomosti.ru, кэш 5мин
- Fallback-карточки без захардкоженных фото (image: '')

**SSG Prerender для SEO:**
- `src/AppSSG.vue` — минимальная версия для пререндера (все 3 темы)
- `src/entry-server.ts` — серверный entry (@vue/server-renderer)
- `scripts/prerender.js` — вставляет HTML в dist/index.html
- Инлайн-скрипт в `<head>`: рандом темы до первого рендера
- CSS: `[data-ssg-theme]` скрывает неактивные, без JS показывает bear
- Build: `vite build && vite build --ssr && node scripts/prerender.js`

**Ссылки на приложения:**
- RuStore → `redirect.appmetrica.yandex.com/serve/101826322610875925`
- AppStore → `redirect.appmetrica.yandex.com/serve/173883918209080136`
- AppGallery → `redirect.appmetrica.yandex.com/serve/390056699366506369`
- Google Play → `redirect.appmetrica.yandex.com/serve/462114291743470927`

### Статус (2026-04-01)
Проверено на production (animal-weld.vercel.app): все 7 BP, RSS через свой API, SSG для SEO.
Осталось: SSR-планирование (ISR для карточек), финальная проверка дизайнером.
